...
- CI/registry: image now builds via **GitHub Actions** and publishes to
  **GHCR** (`ghcr.io/charlestephen/hassio-addons-linkding-{arch}`), replacing
...
- Store bookmarks + config in the app's dedicated **`addon_config`** dir
  (mounted at `/config`; host `/addon_configs/<slug>_linkding`, reachable via the
  Samba/SSH apps) instead of the opaque `/data` volume, keeping the bookmark
  DB out of HA-config backups.

## 1.0.1

- Fix `attempt to write a readonly database` (HTTP 500 on every request). uWSGI
  drops to `www-data`, but the app's `/data` symlink defeated the upstream
...
- Fix app store warning `expected str for dictionary value @
...
- Initial release: Linkding bookmark manager on `ghcr.io/sissbruecker/linkding:latest-alpine`,
  wrapped with the HA app framework (s6-overlay grafted from the HA Alpine base).
- SQLite by default, persisted to the app's `/data` volume.
